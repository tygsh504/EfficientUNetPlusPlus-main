...
def run_prediction_on_disease(disease_name, net, device, params, flops):
    ...
    Folder layout expected (same as testing.py):
        <BASE_DATA_PATH>/<disease_name>/Infer_Ori/   ← images
        <BASE_DATA_PATH>/<disease_name>/Infer_GT/    ← ground-truth masks
    """
    img_dir  = os.path.join(BASE_DATA_PATH, disease_name, "images") #Infer_Ori
    mask_dir = os.path.join(BASE_DATA_PATH, disease_name, "masks") #Infer_GT
    ...
    # ── Use PaddyBinaryDataset exactly as train.py does ──────────────────────
    dataset = PaddyBinaryDataset(img_dir, mask_dir)
...
if __name__ == '__main__':
    ...
    # ── Load model (same methodology as testing.py) ───────────────────────────
    try:
    ...
    # ── Aggregate across all diseases (same as testing.py) ───────────────────
    if all_results:
